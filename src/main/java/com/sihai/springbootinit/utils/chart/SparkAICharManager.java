package com.sihai.springbootinit.utils.chart;
...
@Service
public class SparkAICharManager extends WebSocketListener {
    ...
    // 构造函数
    public SparkAICharManager(String userId, Boolean wsCloseFlag) {
        this.userId = userId;
        this.wsCloseFlag = wsCloseFlag;
    }
    public SparkAICharManager(){
    ...
    public String sendMessageAndGetResponse(String message,Integer sleepTime) throws Exception {
        ...
        while (true){
            if(totalFlag){
                ...
                for (int i = 0; i < 1; i++) {
                    totalAnswer="";
                    WebSocket webSocket = client.newWebSocket(request, new SparkAICharManager(i + "",
                            false));
                }
            }else{
